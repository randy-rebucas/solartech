...
import type { LoginRequest } from '@solartech/shared';
...
export function LoginForm() {
  ...
  async function onSubmit(data: FormData) {
    setServerError(null);
    try {
      const result = await loginWithApi(data as LoginRequest);

      if (result.requiresMfa && result.mfaSessionToken) {
        sessionStorage.setItem('mfa_session', result.mfaSessionToken);
  ...
  return (
    ...
    >
      <div className="mb-6">
        ...
        <p className="text-sm text-slate-400 mt-1">Sign in to your SolarTech account</p>
      </div>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
        ...
        <div>
          ...
          <div className="relative">
            ...
            <input
              ...
              placeholder="••••••••"
              className={cn(
                'w-full pl-10 pr-10 py-2.5 rounded-lg bg-white/5 border text-white placeholder:text-slate-500',
                'focus:outline-none focus:ring-2 focus:ring-solar-500/50 transition-all',
        ...
        <button
          ...
          className={cn(
            ...
            'bg-gradient-solar hover:opacity-90 active:scale-[0.98]',
            'shadow-glow focus:outline-none focus:ring-2 focus:ring-solar-500/50',
      ...
      <p className="mt-6 text-center text-sm text-slate-400">
        ...
        <Link href="/register" className="text-solar-400 hover:text-solar-300 font-medium transition-colors">
          Create one
        </Link>
      </p>
    </motion.div>
...
function OAuthButton({ provider, label }: { provider: string; label: string }) {
  ...
  return (
    <button
      type="button"
      className={cn(
        'flex items-center justify-center gap-2 px-4 py-2.5 rounded-lg border border-white/10',
        'bg-white/5 text-sm text-slate-300 hover:bg-white/10 transition-all active:scale-[0.98]',
      )}
      onClick={() => {
